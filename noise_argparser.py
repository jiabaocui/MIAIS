...
class NoiseArgParser(object):
    def __init__(self):
        super(NoiseArgParser, self).__init__()
    
    def __call__(self, values, namespace):
        layers = []
        #pdb.set_trace()
        split_commands = values.split('+')
        ...
        for command in split_commands:
            ...
            if command[:len('cropout')] == 'cropout':
                ...
                raise ValueError('Command not recognized: \n{}'.format(command))
        setattr(namespace, 'noise', layers)
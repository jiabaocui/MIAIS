...
class Quantization(nn.Module):
    def __init__(self, device=None):
        ...
        self.N = 10
        self.register_buffer('weights', torch.tensor([((-1) ** (n + 1)) / (np.pi * (n + 1)) for n in range(self.N)]))
        self.register_buffer('scales', torch.tensor([2 * np.pi * (n + 1) for n in range(self.N)]))
        for _ in range(4):

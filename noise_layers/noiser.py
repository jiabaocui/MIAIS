        ...
        for layer in noise_layers:
            if type(layer) is str:
                if layer == 'JpegPlaceholder':
                    self.jpegCompress = JpegCompression(device)
                    self.noise_layers.append(self.jpegCompress)
                elif layer == 'QuantizationPlaceholder':
                    self.quantization = Quantization(device)
                    self.noise_layers.append(self.quantization)
                else:

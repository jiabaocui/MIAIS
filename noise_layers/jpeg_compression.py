...
class JpegCompression(nn.Module):
    def __init__(self, device, yuv_keep_weights = (25, 9, 9)):
        ...
        self.register_buffer('dct_conv_weights', torch.tensor(gen_filters(8, 8, dct_coeff), dtype=torch.float32))
        self.dct_conv_weights.unsqueeze_(1)
        self.register_buffer('idct_conv_weights', torch.tensor(gen_filters(8, 8, idct_coeff), dtype=torch.float32))
    ...
    def create_mask(self, requested_shape):
        if self.jpeg_mask is None or requested_shape > self.jpeg_mask.shape[1:]:
            self.jpeg_mask = torch.empty((3,) + requested_shape)
    ...
    def apply_conv(self, image, filter_type: str):
        ...
        if filter_type == 'dct':
            filters = self.dct_conv_weights.to(image.device)
        elif filter_type == 'idct':
            filters = self.idct_conv_weights.to(image.device)
        else:
    ...
    def forward(self, noised_and_cover):
        ...
        mask = self.get_mask(image_dct.shape[1:]).to(image_dct.device)
